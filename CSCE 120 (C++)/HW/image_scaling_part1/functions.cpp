...
double map_coordinates(unsigned int source_dimension, unsigned int target_dimension, unsigned int pixel_coordinate){
  double scaled = ((static_cast<double>(source_dimension)-1) / (target_dimension-1)) * pixel_coordinate;
  return scaled;
}
...
void scale_image(Pixel sourceImage[][MAX_HEIGHT], unsigned int sourceWidth, unsigned int sourceHeight,
  ...
  for (unsigned int i = 0; i < targetWidth; i++){
    for (unsigned int j = 0; j < targetHeight; j++){
      double x = map_coordinates(sourceWidth, targetWidth, i);
      double y = map_coordinates(sourceHeight, targetHeight, j);

      targetImage[i][j] = bilinear_interpolation(sourceImage, sourceWidth, sourceHeight, x, y);
    }
  }
}